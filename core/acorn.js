...
define(function (require, exports, module) {
		...
		for (var kw in keywordTypes)
				exports.tokTypes[kw] = keywordTypes[kw];
		...
		function initTokenState() {
				...
				if (hack)
						hack.initTokenState(hack, tokPos, input);
		}
		...
		function finishToken(type, val) {
				...
				// token tree output hack
				if (hack)
						hack.finishToken(hack, type, val, input, tokStart, tokEnd, tokPos);
		}
		...
		function parseFunction(node, isStatement) {
				if (tokType === _name)
						node.id = parseIdent();		// hack! used to make acorn parse function(){}
