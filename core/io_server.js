...
define(function (require) {
		...
		function ioServer(ssl) {
				...
				// process io channel
				function chan(req) {
						if (req.url.indexOf('/io_') != 0)
								return;
						var m = req.url.split('_');
						if (m[2] != hs.pass)
								return console.log('invalid password in connection', m[2], hs.pass);
						var c = ch[m[1]];
						if (c)
								return c;
				...
				function staticServe(req, res) {
						...
						if (name == '/') {
								...
								if (hs.packaged == 1) {
										...
										res.end('<html><head><meta http-equiv=\'Content-Type\' CONTENT=\'text/html; charset=utf-8\'><title></title>' + '</head><body style=\'background-color:black\' define-main=\'' + hs.main + '\'>' + '<script src=\'/core/define.js\'></script></body></html>');
										return;
								} else if (hs.packaged) {
										var pkg = '';
										var files = {};
						...
						fs.exists(file, function (x) {
								...
								fs.readFile(file, function (err, data) {
										...
										if (hs.process)
												data = hs.process(file, data, type);
										res.writeHead(200, { 'Content-Type': type });
										res.write(data);
										res.end();
								});
								if (hs.watcher)
										hs.watcher.watch(file);
						});
				}
				...
				function handler(req, res) {
						var c = chan(req);
						if (c && c.handler(req, res))
								return;
						if (hs.proxy)
								return proxyServe(req, res);
