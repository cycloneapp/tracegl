...
define(function () {
		...
		function named(a, f) {
				...
				if (!f._c)
						f._c = f.toString();
				if (!f._n)
						f._n = f._c.match(/function.*?\((.*?)\)/)[1].split(',');
				var n = f._n;
				if (a.length > n.length)
						throw new Error('Argument list mismatch, ' + a.length + ' instead of ' + n.length);
				var g = {};
		...
		function mt(s, h) {
				...
				for (var i = 1; i < m.length; i++) {
						...
						var x = a >>> 16, y = a & 65535;
						var c = b >>> 16, d = b & 65535;
				...
				function nx(a) {
						...
						if (i >= m.length) {
								...
								do {
										v = m[k] & 2147483648 | m[k + 1] & 2147483647;
										m[k] = m[k + M] ^ v >>> 1 ^ (v & 1 ? 2567483615 : 0);
								} while (++k < N - M);
								do {
										v = m[k] & 2147483648 | m[k + 1] & 2147483647;
										m[k] = m[k + M - N] ^ v >>> 1 ^ (v & 1 ? 2567483615 : 0);
								} while (++k < N - 1);
								v = m[N - 1] & 2147483648 | m[0] & 2147483647;
								m[N - 1] = m[M - 1] ^ v >>> 1 ^ (v & 1 ? 2567483615 : 0);
								i = 0;
						}
						v = m[i++];
						v ^= v >>> 11, v ^= v << 7 & 2636928640, v ^= v << 15 & 4022730752, v ^= v >>> 18;
						if (a !== undefined) {
								v = ((a >>> 5) * 67108864 + (v >>> 6)) / 9007199254740992;
								if (p)
										return p[Math.round(v * (t - 1))];
								return v;
						}
						return nx(v);
				}
		...
		function sha1hex(m) {
				...
				function lsb(v) {
						var s = '', i, vh, vl;
						for (i = 0; i <= 6; i += 2)
								vh = v >>> i * 4 + 4 & 15, vl = v >>> i * 4 & 15, s += vh.toString(16) + vl.toString(16);
						return s;
				}
				function hex(v) {
						var s = '', i, j;
						for (i = 7; i >= 0; i--)
								j = v >>> i * 4 & 15, s += j.toString(16);
						return s;
				}
				function utf8(s) {
						s = s.replace(/\r\n/g, '\n');
				...
				var v = 1732584193, w = 4023233417, x = 2562383102, y = 271733878, z = 3285377520;
				var a, b, c, d, e, t;
				var l = m.length;
				var wa = [];
				for (i = 0; i < l - 3; i += 4)
				...
				if (r == 0)
						...
						i = m.charCodeAt(l - 1) << 24 | 8388608;
				else if (r == 2)
						i = m.charCodeAt(l - 2) << 24 | m.charCodeAt(l - 1) << 16 | 32768;
				else
						i = m.charCodeAt(l - 3) << 24 | m.charCodeAt(l - 2) << 16 | m.charCodeAt(l - 1) << 8 | 128;
				wa.push(i);
				while (wa.length % 16 != 14)
						wa.push(0);
				wa.push(l >>> 29);
				wa.push(l << 3 & 4294967295);
				for (bs = 0; bs < wa.length; bs += 16) {
						...
						a = v, b = w, c = x, d = y, e = z;
						for (i = 0; i <= 19; i++)
								t = rl(a, 5) + (b & c | ~b & d) + e + u[i] + 1518500249 & 4294967295, e = d, d = c, c = rl(b, 30), b = a, a = t;
						for (i = 20; i <= 39; i++)
								t = rl(a, 5) + (b ^ c ^ d) + e + u[i] + 1859775393 & 4294967295, e = d, d = c, c = rl(b, 30), b = a, a = t;
						for (i = 40; i <= 59; i++)
								t = rl(a, 5) + (b & c | b & d | c & d) + e + u[i] + 2400959708 & 4294967295, e = d, d = c, c = rl(b, 30), b = a, a = t;
						for (i = 60; i <= 79; i++)
								t = rl(a, 5) + (b ^ c ^ d) + e + u[i] + 3395469782 & 4294967295, e = d, d = c, c = rl(b, 30), b = a, a = t;
						v = v + a & 4294967295;
						w = w + b & 4294967295;
						x = x + c & 4294967295;
						y = y + d & 4294967295;
