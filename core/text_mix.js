...
define(function (require, exports) {
		...
		exports.viewport = function (b) {
				...
				b.view = function (x, y, p, event, center) {
						...
						if (!p || p == 1) {
								// scroll down
								if ((d = y + c - (-s.y + (s.o.h - s.gy) / s.sy - 1)) > 0)
										v.ds(d);
						}
						if (!p || p == 2) {
								// scroll up up 
						...
						// scroll right
						if (!event) {
								if ((d = x + 2 - (-s.x + (s.o.w - s.gx) / s.sx)) > 0)
										h.ds(d);
								// scroll left
								if ((d = x - -s.x) < 0)
										h.ds(d);
				...
				(function (p) {
						...
						p.merge = function (o) {
								...
								while (c) {
										...
										var cf = 0;
										if ((cv - cy || cu - cx) > 0)
												cu = c.x, cv = c.y, cx = c.u, cy = c.v, cf = 1;
										var d = s.l.first();
										while (d) {
												var m = d._d;
												// order points
						...
						while (t) {
								...
								if (y >= j && v <= j + l) {
										...
										for (var i = fn.max(0, v - j); i + j <= y && i < l; i++) {
												...
												// adjust rect
												if (i + j == v)
														xs = x1 = fn.min(x2, u), t.ld && (c.d = t.ld[i]);
												// adjust begin at first line
												if (i + j == y)
														xe = x2 = fn.min(x2, x), xt = 1, t.ld && (c.d = t.ld[i]);
												else
														x2 += 1;
												// include newline
												if (v == y && x2 < x1)
														xs = x2 = x1, x1 = xe, xe = x2;
												// flip em
												// corner flagging
												var fl = 0, of = pf;
												if (p1 == x1)
