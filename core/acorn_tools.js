...
define(function (require, exports, module) {
		...
		exports.dump = function (o, t, r) {
				...
				for (var k in o)
						if (o.hasOwnProperty(k)) {
								if (k == 'parent' || k == 'tokens' || k == 'start' || k == 'end' || k == 'token' || k == 'loc')
										continue;
								if (k == 'token') {
										s += '\n' + t + 'token: ' + o[k].t;
										continue;
		...
		var sTab = {
						...
						IfStatement: function (n) {
								return 'if(' + sExp(n.test) + ')' + sExp(n.consequent) + sSep + (n.alternate ? 'else ' + sExp(n.alternate) + sSep : '');
						},
						ThrowStatement: function (n) {
								return 'throw ' + sExp(n.argument);
						},
						TryStatement: function (n) {
								return 'try ' + sExp(n.block) + sSep + sBlk(n.handlers) + sSep + (n.finalizer ? 'finally ' + sBlk(n.finalizer) : '');
						},
		...
		function initTokenState(hack, tokPos, input) {
				if (tokPos != 0)
						hack.tokTree.w = input.slice(0, tokPos);
		}
		function finishToken(hack, type, val, input, tokStart, tokEnd, tokPos) {
				...
				if (type == types.braceL || type == types.bracketL || type == types.parenL) {
						...
						if (hack.noclose) {
								if (!tokTree._e._u)
										delete tokTree._c, delete tokTree._e;
								else
										delete tokTree._e._u._d;
						}
						if (tokTree._p)
								tokTree = tokTree._p;
				}
