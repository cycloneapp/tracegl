...
function define(id, fac) {
		...
		if (typeof process !== 'undefined') {
				...
				module.exports = global.define = function (id, fac) {
						...
						var req = function (m, id) {
										...
										if (global.define.log && f.indexOf('/') != -1)
												process.stderr.write('<[<[' + f + ']>]>');
										return require(f);
								}.bind(this, m);
						if (typeof fac !== 'function')
								return m.exports = fac;
		...
		def.mkreq = function (base) {
				...
				localreq.reload = function (i, cb) {
						...
						script(id, 'reload', function () {
								delete def.module[id];
								// cause reexecution of module
								cb(req(i, base));
						});
				};
		...
		def.main = document.body.getAttribute('define-main') || window.location.pathname.replace(/^(.*\/)(.*?)/, '$2').replace('.html', '');
