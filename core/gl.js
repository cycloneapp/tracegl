...
define(function (require, exports, module) {
		...
		(function () {
				...
				this.draw = function (b) {
						...
						if (b.up)
								gl.bufferData(gl.ARRAY_BUFFER, b.$va, gl.STATIC_DRAW);
						var vt = b.$vt;
						// vertex types
						for (var i in vt) {
								var t = vt[i];
						...
						if (sd.i) {
								...
								if (b.up)
										gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, b.$ia, gl.STATIC_DRAW);
				...
				this.alloc = function (sc, ob) {
						...
						if (ob) {
								...
								if (sd.i)
										b.$ib = _delib.pop() || gl.createBuffer();
								// indexbuffer
								b.lo = 0;
								// low used
								b.hi = 0;
		...
		// \____________________________________________/
		var fnid_c = 1;
		// the function id counter
		var fnid_o = {};
		// id to function string lookup
		var fnid_tc = {};
		// tracecache, used for fast shader hashing
		var fnid_rc = {};
		// reverse function lookup
		var fnid_ev = {};
		// js function evaluation cache
		...
		gl.getShader = function (sd, dn) {
				...
				if (vu > 0)
						yf += 'varying vec' + (vu > 1 ? vu : 2) + ' v_' + vs + ';\n';
				...
				if (sd.p)
						ob += ' gl_PointSize = ' + expr(sd.p, 0, 0, ns) + ';\n';
				for (var i in ad) {
						if (i in illegal_attr)
								throw new Error('Cannot name an attribute hi,lo or i.' + i);
						od += 'attribute ' + ad[i] + ' ' + i + ';\n';
				}
				...
				gl.compileShader(gv);
				if (!gl.getShaderParameter(gv, gl.COMPILE_STATUS))
						throw new Error(gl.getShaderInfoLog(gv) + '\n' + vs);
				sh.$sf = {};
				for (var i in sf)
				...
				for (i in ud) {
						...
						if (i in sh)
								throw new Error('Cannot use uniform with name ' + i);
				...
				function frag(gv, fs) {
						...
						gl.compileShader(gf);
						if (!gl.getShaderParameter(gf, gl.COMPILE_STATUS))
								throw new Error(gl.getShaderInfoLog(gf) + '\n' + fs);
				...
				function expr(f, a, lv, ns) {
						...
						var id = f._i;
						if (!id)
								f._i = id = fnid_o[c] || (fnid_o[c] = fnid_c++);
						var tc = fnid_tc[id] || (fnid_tc[id] = {});
						// trace cache
						fnid_rc[id] = f;
						var p = acorn_tools.parse(c, {
										noclose: 1,
										compact: 1
								}).tokens._c;
						var ma = {};
						...
						if (p.t.match(/^function/)) {
								if (a) {
										...
										// count args
										c = a.length - c - 1;
										// smear (1,2)->(a,b,c) to (a=1,b=1,c=2)
										for (var i = p._c; i; i = i._d)
												if (i.name)
														ma[i.t] = a[++c < 0 ? 0 : c];
								}
						...
						function subexpr(i, f, lv, ns) {
								...
								var e = f._e;
								if (!e)
										f._e = e = c.indexOf('_fw_') != -1 ? 3 : c.indexOf('return_') != -1 ? 2 : c.indexOf('return') != -1 ? 4 : 1;
								var ar;
								// args
								if (i._d && i._d.parenL) {
								...
								if (e == 1) {
										...
										var o = i.t.indexOf('.');
										if (o != -1)
												i.t = i.t.slice(0, o) + '_' + i.t.slice(o + 1);
										if (!fd[i.t]) {
												// not defined yet
												fd[i.t] = 1;
						...
						function subfn(f, t, ns) {
								...
								while (i) {
										...
										if (i.name && i._d && i._d.semi) {
												...
												while (j && !j.semi)
														j = j._d;
												if (!j)
														throw new Error('assignment without terminating ; found');
												...
												if (o != 0 && gt.types[y = i.t.slice(0, o)]) {
														...
														var k = i.t.slice(o + 1);
														if (rt && k != rt)
																throw new Error('please use one return type in ' + t);
														rt = k;
												...
												if (!p1 || !p2 || !p3 || o == -1)
														throw new Error('for loop without init declaration');
								...
								if (!rt)
										throw new Error('no returntype for ' + t);
								os = rt + ' ' + os;
								return os;
						}
								...
								while (i && i != x) {
										...
										if (i.num && i.t.indexOf('.') == -1) {
												...
												if (t in ma)
														...
														var k = i.t.slice(o + 1);
														if (!sd.t)
																throw new Error('theme object not supplied to compiler for ' + i.t);
														if (!(k in sd.t))
																throw new Error('color not defined in theme: ' + i.t);
														if (!('T' in ud)) {
																// set up T uniform
																pd.T = ud.T = 'sampler2D';
																tl.T = sd.t;
														...
														var to = typeof j;
														gl.regvar(k);
														// hook to allow ui node prototype to update
												...
												if (!in_f)
														throw new Error('texture not supported in vertex shader');
												if (!(i.t in ts)) {
														var o = ts[i.t] = '_' + ti++;
														ud[o] = 'sampler2D';
														var t = i.t.slice(1, -1);
				...
				function expand(i) {
						...
						while (i) {
								if (i.num && i.t.indexOf('.') == -1)
										...
										throw new Error('texture not supported in JS expressions');
								if (i.comma)
										a.push(s), s = '';
								else if (i.parenL)
										s += '(' + expand(i._c).join(',') + ')';
		...
		function gt() {
				...
				function co(i, a, o, m, s) {
						...
						if (t == 'number')
								...
								if ('r' in i)
										a[o] = (i.r * 255 & 255) << 24 | (i.g * 255 & 255) << 16 | (i.b * 255 & 255) << 8 | i.a * 255 & 255;
								else
										a[o] = (i.x * 255 & 255) << 24 | (i.y * 255 & 255) << 16 | (i.z * 255 & 255) << 8 | i.w * 255 & 255;
						} else if (t == 'array')
								a[o] = (i[0] * 255 & 255) << 24 | (i[1] * 255 & 255) << 16 | (i[2] * 255 & 255) << 8 | i[3] * 255 & 255;
						else {
								var i = parseColor(i);
								a[o] = (i.r * 255 & 255) << 24 | (i.g * 255 & 255) << 16 | (i.b * 255 & 255) << 8 | i.a * 255 & 255;
						}
		...
		function packColor(c) {
				return c.a * 255 << 24 & 4278190080 | c.b * 255 << 16 & 16711680 | c.g * 255 << 8 & 65280 | c.r * 255 & 255;
		}
		gl.parseColor = parseColor;
		gl.packColor = packColor;
		// ABGR
		gl.uniform1ic = function (i, u) {
				gl.uniform4f(i, (u & 255) / 255, (u >> 8 & 255) / 255, (u >> 16 & 255) / 255, (u >> 24 & 255) / 255);
		};
