...
define(function (require, exports) {
		...
		cm.scroll = function (b, k, v) {
				...
				b.p = function (n) {
						if (n != b)
								return;
						// press event from child
						r = b.mv;
						// one page up/down scrolling
						var l = ui.rel(k);
		...
		cm.select = function (b) {
				...
				b.k = function () {
						if (!s)
								se(b._c);
						if (s && ui.key.i == 'up' && s._u)
								se(s._u);
						if (s && ui.key.i == 'down' && s._d)
								se(s._d);
						if (s && ui.key.i == 'pageup')
								se(ui.count(s, -10));
						if (s && ui.key.i == 'pagedown')
								se(ui.count(s, 10));
						if (s && ui.key.i == 'home')
								se(ui.first(s));
						if (s && ui.key.i == 'end')
		...
		cm.drag = function (b, c) {
				...
				c.p = function () {
						...
						if (ui.bubble(c._p, 'p'))
								return 1;
						// give parent option to capture first
						ui.cap = c;
						mx = ui.mx, my = ui.my;
						sx = b.x;
						sy = b.y;
		...
		cm.resize = function (b) {
				...
				b.m = function (n) {
						if (ui.cap == b) {
								...
								var dy = ui.my - my;
								if (bx == 1)
										b.w = fn.min(b.maxw || 9999, fn.max(b.minw || 50, ov.w - dx)), b.x = ov.x - (b.w - ov.w);
								if (bx == 2)
										b.w = fn.min(b.maxw || 9999, fn.max(b.minw || 50, ov.w + dx));
								if (by == 1)
										b.h = fn.min(b.maxh || 9999, fn.max(b.minh || 50, ov.h - dy)), b.y = ov.y - (b.h - ov.h);
								if (by == 2)
										b.h = fn.min(b.maxh || 9999, fn.max(b.minh || 50, ov.h + dy));
								ui.redraw(b);
								return;
						}
						//if(n != b) return
		...
		cm.edit = function (b, t, c, s, m) {
				...
				b.u = function () {
						...
						for (var q = p; q < t.t.length; q++)
								if (t.t.charCodeAt(q) != 32)
										break;
						for (var r = p; r >= 0; r--)
								if (t.t.charCodeAt(r) != 32)
										break;
						p = p - r < q - p ? r : q;
						for (var e = p; e < t.t.length; e++)
								if (t.t.charCodeAt(e) == 32)
										break;
						for (var s = p; s >= 0; s--)
								if (t.t.charCodeAt(s) == 32)
										break;
						mark(s + 1, e);
						if (!ct)
								ct = fn.dt();
