...
define(function (require) {
		...
		var head;
		function mkHead() {
				// Used to be a newline stripper, but it didn't do well so now it's just a noop
				function strip(i) {
						return i;
				}
				// trace impl
		...
		function instrument(file, src, iid, opt) {
				...
				function instrumentFn(n, name, isRoot, parentId) {
						...
						if (n.body && n.body.body && n.body.body[0] && n.body.body[0].type === 'ExpressionStatement' && n.body.body[0].expression.type === 'Literal' && n.body.body[0].expression.value === 'no tracegl')
								return;
						var fnid = id;
						if (!isRoot) {
								...
								dict[id++] = {
										x: n.body.loc.start.column,
										y: n.body.loc.start.line,
						...
						function needSemi(p, pos) {
								...
								return p.node.type === 'ExpressionStatement' && (p.up.node.type === 'BlockStatement' || p.up.node.type === 'Program' || p.up.node.type === 'SwitchCase');
						}
             ...
             //addBlock(n.alternate)
								...
								AssignmentExpression: function (n, p) {
										if (n.right.type !== 'Literal' && n.right.type !== 'FunctionExpression' && n.right.type !== 'ObjectExpression')
												addAssign(n.left.loc, n.right.start);
								},
								CallExpression: function (n, p) {
										// only if we are the first of a SequenceExpression
										if (p.node.type === 'SequenceExpression' && p.node.expressions[0] === n)
												p = p.up;
										...
										if (n.callee.type === 'MemberExpression') {
												if (n.callee.property.name === 'call')
														ce = 1;
												if (n.callee.property.name === 'apply')
														ce = 2;
										}
										dict[id++] = {
						...
						if (isRoot) {
								...
								dict[id++] = {
										x: n.body.loc.end.column,
										y: n.body.loc.end.line,
										ret: fnid
								};
						}
				}
