...
define(function (require) {
		...
		//|  Styling
		//\____________________________________________/   
		var ft1 = ui.gl.sfont(navigator.platform.match(/Mac/) ? '12px Menlo' : '12px Lucida Console');
		function codeBubble(g) {
				...
				var title = ui.rect({
								sel: 0,
								f: 'mix(t.codeHover,t.codeMark,n.sel)',
								y: 10,
								h: 30,
								x: 10,
								w: 'p.w - 20'
...
*/
         ...
         //body.drawShadows()
				...
				bg.setTitle = function (m, tdb) {
						...
						// cause height to be recalculated in v_
						title.v_();
				...
				function formatCall(m, v, tdb) {
						...
						if (m.r && m.r.v)
								v.addFormat(' ' + tdb.fmtCall(m.r), tdb.colors);
				}
				body.markerHover = function (m) {
						// make sure we only process on change
						if (ui.mx == lx && ui.my == ly && m == lc)
								return;
