...
define(function (require) {
		...
		// the nodejs loader
		if (process.argv[2] && process.argv[2].indexOf('-l') == 0)
				return nodeLoader();
		function nodeLoader() {
				...
				for (var i = 2; i < arg.length; i++) {
						...
						if (a.charAt(0) == '-') {
								...
								if (a.indexOf('-gz') == 0) {
										...
										if (d == -1)
												return usage('No port specified');
										uiport = parseInt(b);
								...
								if (a.match(/\.gz$/i))
										return gzPlaybackMode(f, a, sender);
								if (a.match(/\.js$/i) || isfile)
										return nodeJSMode(f, a, arg.slice(i + 1), sender);
								if (a.match(/^https?/))
										return proxyMode(f, tgtport, bind, a, sender);
						...
						if (fs.existsSync(ed[k].bin)) {
								...
								for (var i = 0; i < args.length; i++) {
										narg[i] = args[i].replace(/\$(\w+)/g, function (m, a) {
												if (!a in rep)
														console.log('Opening editor: argument not supported ' + a);
												return rep[a];
										});
								}
		...
		function browserJSMode(filter, port, bind, root, sender) {
				...
				tgt.process = function (file, data, type) {
						if (type != 'application/javascript')
								return data;
						if (filter.active && filter(file))
								return data;
						// cache
						if (fileCache[file])
								return fileCache[file].output;
						// lets use trace
		...
		function nodeJSMode(filter, file, args, sender) {
				...
				var child = cp.spawn(process.execPath, args, { stdio: stdio });
				// stderr datapath
				var sp = streamParser(sender, function (d) {
								process.stderr.write(d);
						});
				if (child.stderr)
						child.stderr.on('data', sp);
				// ipc datapath
